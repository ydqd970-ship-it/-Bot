...
def fetch_stock_data(ticker: str) -> dict | None:
    """株価データとテクニカル指標を取得（リトライあり）"""
    df = None
    for attempt in range(3):
        try:
            time.sleep(attempt * 2)
            # Ticker().history()方式（download()がブロックされる場合の代替）
            stock = yf.Ticker(ticker)
            raw = stock.history(period="1y", auto_adjust=True)
            print(f"[DEBUG] {ticker} Ticker方式 取得行数:{len(raw)} 空:{raw.empty}")
            if raw.empty:
                # download()でも試行
                raw = yf.download(ticker, period="1y",
                                  auto_adjust=True, progress=False, threads=False)
                print(f"[DEBUG] {ticker} download方式 取得行数:{len(raw)} 空:{raw.empty}")
            if raw.empty:
                print(f"[WARN] {ticker} 両方式で空データ（attempt {attempt+1}）")
                continue
            df_tmp = _normalize_df(raw).dropna()
            print(f"[DEBUG] {ticker} 正規化後:{len(df_tmp)}行 列:{list(df_tmp.columns)}")
            if len(df_tmp) >= 30:
                df = df_tmp
                break
            print(f"[WARN] {ticker} データ不足:{len(df_tmp)}行")
        except Exception as e:
            print(f"[ERROR] {ticker} 取得失敗(attempt {attempt+1}): {e}")
    if df is None:
        return None
    try:
...
def analyze_ticker(ticker: str) -> str:
    ...
    if data is None:
        ...
        try:
            info = yf.Ticker(ticker).info
            name = info.get("longName") or info.get("shortName") or ticker
            return (f"❌ `{ticker}`（{name}）のデータ取得に失敗しました。\n"
                    f"> 原因：データが少ないか、ティッカーが無効な可能性があります。\n"
                    f"> 日本株は末尾に `.T` が必要です（例：`6954.T`）")
        except Exception:
            return (f"❌ `{ticker}` のデータを取得できませんでした。\n"
                    f"> ティッカーを確認してください（例：`6954.T`、`AAPL`）")
